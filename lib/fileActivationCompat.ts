...
export async function activateFile(
  fileId: string,
  userId: string
): Promise<{ success: boolean; message: string; viewName?: string }> {
  try {
    console.log(`[FileActivation] Activating file ${fileId} for user ${userId}`);

    // Check if file exists and belongs to the user
    const fileExists = await checkFileExists(fileId, userId);
    if (!fileExists) {
      console.log(`[FileActivation] File ${fileId} not found or does not belong to user ${userId}`);
      return { success: false, message: "File not found or does not belong to user" };
    }
    ...
    const sanitizedFilename = path.basename(fileInfo.filename, path.extname(fileInfo.filename))
      .replace(/[^a-zA-Z0-9]/g, "_");
    const viewName = `data_${sanitizedUserId}_${sanitizedFilename}`;
    console.log(`[FileActivation] Creating view ${viewName} for file ${fileId} (${fileInfo.filename})`);

    // Check if view_metadata table exists
    const viewMetadataExists = await checkTableExists("view_metadata");
    console.log(`[FileActivation] Table view_metadata exists: ${viewMetadataExists}`);

    if (viewMetadataExists) {
      // Store view metadata
      await storeViewMetadata(fileId, userId, viewName, fileInfo.filename);
    } else {
      console.log(`[FileActivation] view_metadata table does not exist, using direct view creation`);
    }
    ...
    console.log(`[FileActivation] Successfully attached file ${fileId} to user ${userId} workspace as ${viewName}`);
    ...
    try {
      ...
      console.error(`[FileActivation] Error updating file status:`, statusError);
    }
    return { 
      success: false, 
      message: error instanceof Error ? error.message : "Unknown error during file activation" 
    };
  }
}
 ...
 */
async function checkFileExists(fileId: string, userId: string): Promise<boolean> {
  try {
 ...
 * @returns Promise<boolean>
 */
async function updateFileStatus(fileId: string, status: string): Promise<boolean> {
  try {
    await executeQuery(`
      UPDATE files
      SET status = '${status}'
 ...
 * @returns Promise<boolean>
 */
async function storeViewMetadata(
  fileId: string,
  userId: string,
  viewName: string,
 ...
 * @returns Promise<boolean>
 */
async function createView(fileId: string, viewName: string): Promise<boolean> {
  try {
