...
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'column_mappings') THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_global_schemas_project') THEN
    CREATE INDEX "idx_global_schemas_project" ON "global_schemas"("project_id");
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_schema_columns_global_schema') THEN
    CREATE INDEX "idx_schema_columns_global_schema" ON "schema_columns"("global_schema_id");
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'schema_columns_global_schema_id_name_key') THEN
    CREATE UNIQUE INDEX "schema_columns_global_schema_id_name_key" ON "schema_columns"("global_schema_id", "name");
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_column_mappings_file') THEN
    CREATE INDEX "idx_column_mappings_file" ON "column_mappings"("file_id");
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_column_mappings_global_schema') THEN
    CREATE INDEX "idx_column_mappings_global_schema" ON "column_mappings"("global_schema_id");
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_column_mappings_schema_column') THEN
    CREATE INDEX "idx_column_mappings_schema_column" ON "column_mappings"("schema_column_id");
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'column_mappings_file_id_schema_column_id_key') THEN
    CREATE UNIQUE INDEX "column_mappings_file_id_schema_column_id_key" ON "column_mappings"("file_id", "schema_column_id");
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_column_merges_user') THEN
    CREATE INDEX "idx_column_merges_user" ON "column_merges"("user_id");
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_column_merges_file') THEN
    CREATE INDEX "idx_column_merges_file" ON "column_merges"("file_id");
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'column_merges_user_id_file_id_merge_name_key') THEN
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND table_name = 'global_schemas'
  ) THEN
    ALTER TABLE "global_schemas" ADD CONSTRAINT "global_schemas_project_id_fkey"
    FOREIGN KEY ("project_id") REFERENCES "projects"("id")
  ...
  IF NOT EXISTS (
    ...
    AND table_name = 'schema_columns'
  ) THEN
    ALTER TABLE "schema_columns" ADD CONSTRAINT "schema_columns_global_schema_id_fkey"
    FOREIGN KEY ("global_schema_id") REFERENCES "global_schemas"("id")
    ON DELETE RESTRICT ON UPDATE CASCADE;
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'column_mappings_file_id_fkey'
    AND table_name = 'column_mappings'
  ) THEN
    ALTER TABLE "column_mappings" ADD CONSTRAINT "column_mappings_file_id_fkey"
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'column_mappings_global_schema_id_fkey'
    AND table_name = 'column_mappings'
  ) THEN
    ALTER TABLE "column_mappings" ADD CONSTRAINT "column_mappings_global_schema_id_fkey"
    FOREIGN KEY ("global_schema_id") REFERENCES "global_schemas"("id")
    ON DELETE RESTRICT ON UPDATE CASCADE;
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'column_mappings_schema_column_id_fkey'
    AND table_name = 'column_mappings'
  ) THEN
    ALTER TABLE "column_mappings" ADD CONSTRAINT "column_mappings_schema_column_id_fkey"
    FOREIGN KEY ("schema_column_id") REFERENCES "schema_columns"("id")
    ON DELETE RESTRICT ON UPDATE CASCADE;
  END IF;
  
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'file_data') AND
     NOT EXISTS (
       SELECT 1 FROM information_schema.table_constraints
       WHERE constraint_name = 'file_data_file_id_fkey'
       AND table_name = 'file_data'
     ) THEN
    ALTER TABLE "file_data" ADD CONSTRAINT "file_data_file_id_fkey"
    FOREIGN KEY ("file_id") REFERENCES "files"("id")
    ON DELETE RESTRICT ON UPDATE CASCADE;
  END IF;
